...
class DetailsFragment : Fragment() {
    ...
    private fun initUI() {
        ...
        with(binding) {
            ...
            when (model.name) {
                ...
                com.example.pokedex.mew -> {
                    ...
                    root.setBackgroundColor(getResources().getColor(com.example.pokedex.R.color.mew_clr))
                    weightTv.text = "4 kg"
                    heightTv.text = "0,4 m"
                    typeTv.text = "Electric"
                    typeTv.setBackgroundColor(getResources().getColor(R.color.mew_clr))
                    with(aboutTv) { setTextColor(R.color.mew_clr) }
                    descriptionTv.text =
        ...
        when (model.name) {
            ...
            charmander -> {
                activity?.let {
                    activity?.window?.setStatusBarColor(it.getColor(R.color.charmander_clr))
                }
            }
            mew -> {
                activity?.let {
                    activity?.window
                        ?.setStatusBarColor(it.getColor(R.color.mew_clr))
                }
            }
        }
